    ...
    return (
        <svg viewBox={`0 0 ${w} ${h}`} style={{ width: '100%', height: 'auto', display: 'block' }}>
            ...
            {events.filter((ev: any) => ev.type === 'goal' || ev.type === 'red').map((ev: any, i: number) => {
                ...
                return (<g key={i}><line x1={x} y1={pad.t} x2={x} y2={h - pad.b} stroke={isRed ? 'rgba(239,68,68,0.2)' : ev.side === 'home' ? `${homeColor}33` : `${awayColor}33`} strokeWidth="1" /><circle cx={x} cy={ev.side === 'home' ? pad.t + 3 : h - pad.b - 3} r="3" fill={isRed ? '#ef4444' : ev.side === 'home' ? homeColor : awayColor} stroke="#0e0e0e" strokeWidth="1.5" /><text x={x} y={ev.side === 'home' ? pad.t - 4 : h - pad.b + 12} textAnchor="middle" fill="rgba(255,255,255,0.35)" fontSize="7" fontFamily="'JetBrains Mono', monospace" fontWeight="500">{ev.min}'</text></g>);
            })}
            ...
            {[0, 15, 30, 60, 75, 90].map((m) => (<g key={m}><line x1={toX(m)} y1={h - pad.b} x2={toX(m)} y2={h - pad.b + 3} stroke="rgba(255,255,255,0.07)" strokeWidth="1" /><text x={toX(m)} y={h - 6} textAnchor="middle" fill="rgba(255,255,255,0.12)" fontSize="7" fontFamily="'JetBrains Mono', monospace">{m}</text></g>))}
        </svg>
    );
}
...
function StatRow({ label, home, away, hv, av, idx, homeColor, awayColor }: any) {
    ...
    return (
        <div className="stat-row">
            ...
            <div className="stat-bar stat-bar--home"><div style={{ width: on ? `${hp}%` : '0%', height: '100%', borderRadius: 2, background: hw ? homeColor : 'rgba(255,255,255,0.12)', transition: 'width 0.9s cubic-bezier(0.4,0,0.2,1)' }} /></div>
            <span className="stat-label">{label}</span>
            <div className="stat-bar"><div style={{ width: on ? `${100 - hp}%` : '0%', height: '100%', borderRadius: 2, background: aw ? awayColor : 'rgba(255,255,255,0.12)', transition: 'width 0.9s cubic-bezier(0.4,0,0.2,1)' }} /></div>
...
export default function MatchPage() {
    ...
    if (loading) return (<div className="mp-loading"><style>{`:root { --f-serif: 'Newsreader', Georgia, serif; --f-mono: 'JetBrains Mono', monospace; --f-sans: 'DM Sans', system-ui, sans-serif; } @keyframes spin { to { transform: rotate(360deg); } } .mp-loading { min-height: 100vh; background: #080808; display: flex; flex-direction: column; align-items: center; justify-content: center; gap: 12px; }`}</style><div style={{ width: 20, height: 20, border: '2px solid rgba(255,255,255,0.08)', borderTop: '2px solid rgba(255,255,255,0.4)', borderRadius: '50%', animation: 'spin 0.8s linear infinite' }} /><div style={{ color: 'rgba(255,255,255,0.25)', fontFamily: 'var(--f-mono)', fontSize: 10, letterSpacing: '0.14em', textTransform: 'uppercase' }}>Loading</div></div>);

    if (!match) return (<div style={{ minHeight: '100vh', background: '#080808', display: 'flex', flexDirection: 'column', alignItems: 'center', justifyContent: 'center', gap: 16, color: '#fff' }}><div style={{ fontFamily: 'var(--f-serif)', fontSize: 24, fontWeight: 500 }}>Match not found</div><Link to="/reports" style={{ fontFamily: 'var(--f-mono)', fontSize: 11, color: 'rgba(255,255,255,0.3)', textDecoration: 'none', padding: '8px 16px', border: '1px solid rgba(255,255,255,0.06)', borderRadius: 8 }}>← Back to Reports</Link></div>);
    ...
    return (
        ...
        * { box-sizing: border-box; } ::selection { background: rgba(59,130,246,0.30); }
        ...
        .mp-page { min-height: 100vh; background: #080808; display: flex; align-items: flex-start; justify-content: center; padding: clamp(18px, 3.5vw, 32px) 16px 88px; --cardW: 640px; --cardPad: clamp(16px, 3.2vw, 22px); --section: clamp(16px, 3.2vw, 22px); --border: rgba(255,255,255,0.06); }
        .intel-card { width: 100%; max-width: var(--cardW); background: rgba(14,14,14,0.65); backdrop-filter: blur(32px); -webkit-backdrop-filter: blur(32px); border-radius: 20px; overflow: hidden; border: 1px solid rgba(255,255,255,0.06); box-shadow: inset 0 1px 0 rgba(255,255,255,0.04), 0 8px 32px rgba(0,0,0,0.4), 0 24px 64px rgba(0,0,0,0.6); }
        .topbar { padding: 12px var(--cardPad); display: flex; justify-content: space-between; align-items: center; border-bottom: 1px solid rgba(255,255,255,0.03); background: rgba(255,255,255,0.008); }
        .tabRail { display: flex; gap: 6px; padding: 8px var(--cardPad); border-top: 1px solid rgba(255,255,255,0.03); border-bottom: 1px solid rgba(255,255,255,0.03); background: rgba(255,255,255,0.005); overflow-x: auto; -webkit-overflow-scrolling: touch; }
        .tabRail::-webkit-scrollbar { height: 0; }
        .tab-btn { all: unset; cursor: pointer; font-family: var(--f-mono); font-size: 10px; font-weight: 500; letter-spacing: 0.08em; text-transform: uppercase; padding: 10px 14px; border-radius: 10px; color: rgba(255,255,255,0.22); transition: background 0.2s ease, color 0.2s ease; position: relative; white-space: nowrap; flex: 0 0 auto; }
        ...
        .tab-btn[data-active="true"]::after { content: ''; position: absolute; bottom: 3px; left: 50%; transform: translateX(-50%); width: 16px; height: 2px; border-radius: 1px; background: rgba(255,255,255,0.3); }
        ...
        .stat-head { display: grid; grid-template-columns: 52px 1fr 84px 1fr 52px; align-items: center; margin-bottom: 6px; padding-bottom: 10px; border-bottom: 1px solid rgba(255,255,255,0.03); }
        .stat-row { display: grid; grid-template-columns: 52px 1fr 84px 1fr 52px; align-items: center; border-bottom: 1px solid rgba(255,255,255,0.02); padding: 10px 0; border-radius: 10px; transition: background 0.2s ease; }
        .stat-row:hover { background: rgba(255,255,255,0.02); }
        .stat-num { font-family: var(--f-mono); font-size: 12px; font-weight: 400; color: rgba(255,255,255,0.35); transition: color 0.35s ease; }
        .stat-num[data-win="1"] { color: #fff; font-weight: 650; }
        .stat-label { font-family: var(--f-serif); font-size: 10px; font-weight: 400; color: rgba(255,255,255,0.25); text-align: center; text-transform: uppercase; letter-spacing: 0.06em; font-style: italic; padding: 0 8px; }
        .stat-bar { height: 3px; border-radius: 2px; overflow: hidden; background: rgba(255,255,255,0.03); margin: 0 14px; }
            ...
            <div className="intel-card" style={{ opacity: ready ? 1 : 0, transition: 'opacity 0.5s ease' }}>
                <div className="topbar">
                    <div style={{ display: 'flex', alignItems: 'center', gap: 8 }}>
                        <Link to="/reports" style={{ fontFamily: 'var(--f-mono)', fontSize: 9, color: 'rgba(255,255,255,0.2)', textDecoration: 'none', letterSpacing: '0.06em', padding: '4px 10px', border: '1px solid rgba(255,255,255,0.06)', borderRadius: 8, transition: 'all 0.15s', lineHeight: 1 }}>←</Link>
                        <div style={{ display: 'flex', alignItems: 'center', gap: 6 }}>
                            <svg width="10" height="10" viewBox="0 0 10 10"><circle cx="5" cy="5" r="2.5" fill={HC} opacity="0.6" /></svg>
                            <span style={{ fontFamily: 'var(--f-sans)', fontSize: 10, fontWeight: 700, color: 'rgba(255,255,255,0.3)', textTransform: 'uppercase', letterSpacing: '0.14em' }}>{LEAGUE_LABELS[match.league_id] || LEAGUE_SHORT[match.league_id] || match.league_id}</span>
                        </div>
                    </div>
                ...
                <div className="hero">
                    <div style={{ position: 'absolute', inset: 0, pointerEvents: 'none' }}>
                        <div style={{ position: 'absolute', left: '15%', top: '30%', width: 220, height: 220, borderRadius: '50%', background: HC, opacity: 0.06, filter: 'blur(60px)' }} />
                        <div style={{ position: 'absolute', right: '15%', bottom: '20%', width: 220, height: 220, borderRadius: '50%', background: AC, opacity: 0.04, filter: 'blur(60px)' }} />
                    </div>
                    <div className="heroGrid">
                        <Link to={teamUrl(match.home_team)} className="team-link" style={{ display: 'flex', flexDirection: 'column', alignItems: 'center', gap: 12, animation: 'fadeUp 0.5s ease 0.1s both' }}>
                            {homeLogo ? <img src={homeLogo} alt={match.home_team} style={{ width: 64, height: 64, objectFit: 'contain', filter: 'drop-shadow(0 4px 20px rgba(0,0,0,0.6))' }} /> : <div style={{ width: 64, height: 64, borderRadius: '50%', background: `linear-gradient(135deg, ${HC}, ${HC}88)`, display: 'flex', alignItems: 'center', justifyContent: 'center', fontSize: 24, fontWeight: 'bold', color: '#fff' }}>{match.home_team[0]}</div>}
                            <div style={{ textAlign: 'center' }}><div style={{ fontFamily: 'var(--f-sans)', fontSize: 14, fontWeight: 800, color: '#fff', letterSpacing: '-0.02em' }}>{match.home_team}</div></div>
                        </Link>
                        ...
                        <Link to={teamUrl(match.away_team)} className="team-link" style={{ display: 'flex', flexDirection: 'column', alignItems: 'center', gap: 12, animation: 'fadeUp 0.5s ease 0.15s both' }}>
                            {awayLogo ? <img src={awayLogo} alt={match.away_team} style={{ width: 64, height: 64, objectFit: 'contain', filter: 'drop-shadow(0 4px 20px rgba(0,0,0,0.6))' }} /> : <div style={{ width: 64, height: 64, borderRadius: '50%', background: `linear-gradient(135deg, ${AC}, ${AC}88)`, display: 'flex', alignItems: 'center', justifyContent: 'center', fontSize: 24, fontWeight: 'bold', color: '#fff' }}>{match.away_team[0]}</div>}
                            <div style={{ textAlign: 'center' }}><div style={{ fontFamily: 'var(--f-sans)', fontSize: 14, fontWeight: 800, color: 'rgba(255,255,255,0.62)', letterSpacing: '-0.02em' }}>{match.away_team}</div></div>
                        </Link>
                    </div>
                    {match.venue && <div style={{ position: 'relative', textAlign: 'center', marginTop: 18, animation: 'fadeIn 0.5s ease 0.45s both' }}><span style={{ fontFamily: 'var(--f-serif)', fontSize: 11, fontStyle: 'italic', color: 'rgba(255,255,255,0.12)' }}>{match.venue}{match.attendance ? ` · ${match.attendance.toLocaleString()}` : ''}{match.referee ? ` · ${match.referee}` : ''}</span></div>}
                </div>

                {matchEvents.length > 0 && <div style={{ padding: `0 var(--cardPad) 14px`, animation: 'fadeIn 0.5s ease 0.5s both' }}><div style={{ fontFamily: 'var(--f-mono)', fontSize: 8, fontWeight: 700, color: 'rgba(255,255,255,0.12)', textTransform: 'uppercase', letterSpacing: '0.14em', marginBottom: 6 }}>Match Momentum</div><MomentumArc events={matchEvents} homeColor={HC} awayColor={AC} homeAbbr={homeAbbr} awayAbbr={awayAbbr} /></div>}
                ...
                <div className="contentPad" key={tab}>
                    {tab === 'overview' && <div style={{ animation: 'fadeIn 0.25s ease' }}>
                        {matchEvents.length > 0 && <div style={{ marginBottom: 18 }}>{matchEvents.map((ev: any, i: number) => <div key={i} className="timeline-event" style={{ display: 'flex', alignItems: 'flex-start', gap: 12, borderBottom: '1px solid rgba(255,255,255,0.015)', animation: `fadeUp 0.3s ease ${i * 0.04}s both` }}><span style={{ fontFamily: 'var(--f-mono)', fontSize: 10, fontWeight: 600, color: 'rgba(255,255,255,0.2)', width: 38, textAlign: 'right', flexShrink: 0, paddingTop: 1 }}>{ev.raw}</span><div style={{ width: 3, minHeight: 30, borderRadius: 2, flexShrink: 0, marginTop: 1, background: ev.type === 'red' ? '#dc2626' : ev.type === 'yellow' ? '#eab308' : ev.side === 'home' ? HC : AC, opacity: ev.type === 'goal' ? 0.8 : 0.5 }} /><div style={{ flex: 1 }}><div style={{ display: 'flex', alignItems: 'center', gap: 6 }}><span style={{ fontSize: 12 }}>{ev.type === 'red' ? '🟥' : ev.type === 'yellow' ? '🟨' : '⚽'}</span><span style={{ fontFamily: 'var(--f-sans)', fontSize: 13, fontWeight: 700, color: ev.type === 'red' ? 'rgba(239,68,68,0.8)' : '#fff' }}>{ev.player}</span><span style={{ fontFamily: 'var(--f-mono)', fontSize: 8, fontWeight: 700, color: ev.side === 'home' ? HC : AC, opacity: 0.5, textTransform: 'uppercase', letterSpacing: '0.08em' }}>{ev.side === 'home' ? homeAbbr : awayAbbr}</span></div>{ev.detail && <div style={{ fontFamily: 'var(--f-serif)', fontSize: 11, fontStyle: 'italic', color: 'rgba(255,255,255,0.25)', marginTop: 3 }}>{ev.detail}</div>}</div></div>)}</div>}
                        <div style={{ height: 1, background: 'rgba(255,255,255,0.03)', margin: '6px 0 18px', transformOrigin: 'left', animation: 'expandLine 0.6s ease 0.3s both' }} />
                        <div className="insight-box" style={{ animation: 'fadeUp 0.35s ease 0.35s both' }}><div style={{ fontFamily: 'var(--f-mono)', fontSize: 8, fontWeight: 700, color: HC, textTransform: 'uppercase', letterSpacing: '0.12em', marginBottom: 10, opacity: 0.6 }}>Match Summary</div><p style={{ fontFamily: 'var(--f-serif)', fontSize: 14, color: 'rgba(255,255,255,0.5)', lineHeight: 1.75, margin: 0 }}>{match.home_team} {match.home_score > match.away_score ? 'defeated' : match.home_score < match.away_score ? 'lost to' : 'drew with'} {match.away_team} {match.home_score}–{match.away_score}.{match.home_possession != null && ` ${(match.home_possession > 50 ? match.home_team : match.away_team)} controlled possession at ${Math.max(match.home_possession, match.away_possession || 0).toFixed(1)}%.`}{match.home_shots != null && ` The match produced ${(match.home_shots || 0) + (match.away_shots || 0)} shots with ${(match.home_shots_on_target || 0) + (match.away_shots_on_target || 0)} on target.`}{hasOdds && ` ${match.home_team} closed at ${fmtOdds(match.dk_home_ml)} on DraftKings.`}{spreadRes && ` ATS: ${spreadRes.result === 'covered' ? 'Covered' : spreadRes.result === 'failed' ? 'Failed to cover' : 'Push'} the ${match.dk_spread! > 0 ? '+' : ''}${match.dk_spread} spread.`}</p></div>
                        <div className="kpiGrid" style={{ animation: 'fadeUp 0.35s ease 0.45s both' }}>{[{ val: `${match.home_shots || 0}–${match.away_shots || 0}`, label: 'Total Shots', sub: `${match.home_shots_on_target || 0}–${match.away_shots_on_target || 0} on target` }, { val: `${match.home_possession ?? '-'}%`, label: `${match.home_team.split(' ').pop()} Poss.`, sub: `${match.away_possession ?? '-'}% opp.` }, { val: String(totalGoals), label: 'Total Goals', sub: `${(match.home_corners || 0) + (match.away_corners || 0)} corners` }].map((kpi, i) => <div key={i} className="kpi-card" style={{ padding: '16px 14px', background: 'rgba(255,255,255,0.012)', borderRadius: 12, border: '1px solid rgba(255,255,255,0.03)' }}><div style={{ fontFamily: 'var(--f-serif)', fontSize: 26, fontWeight: 500, color: '#fff', letterSpacing: '-0.03em', lineHeight: 1 }}>{kpi.val}</div><div style={{ fontFamily: 'var(--f-sans)', fontSize: 10, color: 'rgba(255,255,255,0.35)', marginTop: 6, lineHeight: 1.3, fontWeight: 600 }}>{kpi.label}</div><div style={{ fontFamily: 'var(--f-mono)', fontSize: 8, color: 'rgba(255,255,255,0.18)', marginTop: 3 }}>{kpi.sub}</div></div>)}</div>
                    </div>}
                    ...
                    {tab === 'odds' && hasOdds && <div style={{ animation: 'fadeIn 0.25s ease', display: 'flex', flexDirection: 'column', gap: 16 }}>
                        <div className="odds-panel"><div style={{ padding: '10px 16px', borderBottom: '1px solid rgba(255,255,255,0.03)', display: 'flex', justifyContent: 'space-between', alignItems: 'center' }}><span style={{ fontFamily: 'var(--f-mono)', fontSize: 8, fontWeight: 700, color: 'rgba(255,255,255,0.2)', textTransform: 'uppercase', letterSpacing: '0.12em' }}>Closing Moneyline</span><span style={{ fontFamily: 'var(--f-mono)', fontSize: 8, color: 'rgba(255,255,255,0.12)' }}>DraftKings</span></div><div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr 1fr' }}><OddsCell label={match.home_team} odds={match.dk_home_ml} isWin={mlRes === 'home'} borderRight /><OddsCell label="Draw" odds={match.dk_draw_ml} isWin={mlRes === 'draw'} borderRight /><OddsCell label={match.away_team} odds={match.dk_away_ml} isWin={mlRes === 'away'} /></div></div>
                        <div className="odds-panel"><div style={{ padding: '10px 16px', borderBottom: '1px solid rgba(255,255,255,0.03)' }}><span style={{ fontFamily: 'var(--f-mono)', fontSize: 8, fontWeight: 700, color: 'rgba(255,255,255,0.2)', textTransform: 'uppercase', letterSpacing: '0.12em' }}>Against the Spread</span></div><div style={{ padding: '16px 18px' }}><div style={{ display: 'flex', justifyContent: 'space-between', marginBottom: 10, alignItems: 'baseline' }}><span style={{ fontFamily: 'var(--f-sans)', fontSize: 13, color: '#fff', fontWeight: 600 }}>{match.home_team}</span><span style={{ fontFamily: 'var(--f-mono)', fontSize: 13, color: 'rgba(255,255,255,0.5)' }}>{homeSpreadStr} <span style={{ color: 'rgba(255,255,255,0.25)' }}>({fmtOdds(match.dk_home_spread_price)})</span></span></div><div style={{ display: 'flex', justifyContent: 'space-between', marginBottom: 16, alignItems: 'baseline' }}><span style={{ fontFamily: 'var(--f-sans)', fontSize: 13, color: 'rgba(255,255,255,0.6)', fontWeight: 600 }}>{match.away_team}</span><span style={{ fontFamily: 'var(--f-mono)', fontSize: 13, color: 'rgba(255,255,255,0.5)' }}>{awaySpreadStr} <span style={{ color: 'rgba(255,255,255,0.25)' }}>({fmtOdds(match.dk_away_spread_price)})</span></span></div><div style={{ borderTop: '1px solid rgba(255,255,255,0.03)', paddingTop: 14 }}><div style={{ fontFamily: 'var(--f-mono)', fontSize: 13, fontWeight: 700, color: spreadRes?.result === 'covered' ? '#22c55e' : spreadRes?.result === 'failed' ? '#ef4444' : 'rgba(255,255,255,0.3)' }}>{spreadRes?.result === 'covered' ? `✓ ${match.home_team} Covered` : spreadRes?.result === 'failed' ? '✗ Failed to Cover' : 'Push'}{spreadRes?.margin != null && <span style={{ fontWeight: 500, color: 'rgba(255,255,255,0.2)', marginLeft: 8 }}>by {spreadRes.margin}</span>}</div></div></div></div>
                        <div className="odds-panel"><div style={{ padding: '10px 16px', borderBottom: '1px solid rgba(255,255,255,0.03)' }}><span style={{ fontFamily: 'var(--f-mono)', fontSize: 8, fontWeight: 700, color: 'rgba(255,255,255,0.2)', textTransform: 'uppercase', letterSpacing: '0.12em' }}>Over / Under</span></div><div style={{ padding: '16px 18px' }}><div style={{ display: 'flex', justifyContent: 'space-between', marginBottom: 10 }}><span style={{ fontFamily: 'var(--f-sans)', fontSize: 13, color: '#fff', fontWeight: 600 }}>Over {match.dk_total}</span><span style={{ fontFamily: 'var(--f-mono)', fontSize: 13, color: 'rgba(255,255,255,0.5)' }}>{fmtOdds(match.dk_over_price)}</span></div><div style={{ display: 'flex', justifyContent: 'space-between', marginBottom: 16 }}><span style={{ fontFamily: 'var(--f-sans)', fontSize: 13, color: 'rgba(255,255,255,0.6)', fontWeight: 600 }}>Under {match.dk_total}</span><span style={{ fontFamily: 'var(--f-mono)', fontSize: 13, color: 'rgba(255,255,255,0.5)' }}>{fmtOdds(match.dk_under_price)}</span></div><div style={{ borderTop: '1px solid rgba(255,255,255,0.03)', paddingTop: 14 }}><div style={{ fontFamily: 'var(--f-mono)', fontSize: 13, fontWeight: 700, color: totalRes?.result === 'over' ? '#22c55e' : totalRes?.result === 'under' ? '#22c55e' : 'rgba(255,255,255,0.3)' }}>{totalRes?.result === 'over' ? '✓ Over Hit' : totalRes?.result === 'under' ? '✓ Under Hit' : 'Push'}<span style={{ fontWeight: 500, color: 'rgba(255,255,255,0.2)', marginLeft: 8 }}>({totalGoals} total goals)</span></div></div></div></div>
                        <div className="insight-box" style={{ animation: 'fadeUp 0.35s ease 0.25s both' }}><div style={{ fontFamily: 'var(--f-mono)', fontSize: 8, fontWeight: 700, color: 'rgba(255,255,255,0.18)', textTransform: 'uppercase', letterSpacing: '0.12em', marginBottom: 10 }}>Closing Market Summary</div><p style={{ fontFamily: 'var(--f-serif)', fontSize: 13, fontStyle: 'italic', color: 'rgba(255,255,255,0.4)', lineHeight: 1.7, margin: 0 }}>{match.home_team} closed at {fmtOdds(match.dk_home_ml)} ({(impliedProb(match.dk_home_ml!) * 100).toFixed(0)}% implied). The spread was set at {homeSpreadStr} with the total at {match.dk_total} goals. The actual total of {totalGoals} goals {totalRes?.result === 'over' ? 'cleared' : totalRes?.result === 'under' ? 'stayed under' : 'landed exactly on'} the line.</p></div>
                    </div>}

                    {tab === 'lineups' && <div style={{ animation: 'fadeIn 0.25s ease' }}>{[{ label: match.home_team, lineup: match.home_lineup, color: HC, logo: homeLogo }, { label: match.away_team, lineup: match.away_lineup, color: AC, logo: awayLogo }].map((team, ti) => { const players = Array.isArray(team.lineup) ? team.lineup : []; const starters = players.filter((p: any) => p.starter); const subs = players.filter((p: any) => !p.starter && p.subbedIn); return (<div key={ti} style={{ marginBottom: ti === 0 ? 28 : 0 }}><div style={{ display: 'flex', alignItems: 'center', gap: 10, marginBottom: 14 }}>{team.logo && <img src={team.logo} alt="" style={{ width: 20, height: 20, objectFit: 'contain' }} />}<span style={{ fontFamily: 'var(--f-sans)', fontSize: 14, fontWeight: 800, color: ti === 0 ? '#fff' : 'rgba(255,255,255,0.62)' }}>{team.label}</span><span style={{ fontFamily: 'var(--f-mono)', fontSize: 9, color: 'rgba(255,255,255,0.15)', marginLeft: 'auto' }}>{starters.length} starters</span></div>{starters.map((p: any, pi: number) => <PlayerRow key={pi} p={p} color={team.color} delay={pi * 0.025} />)}{subs.length > 0 && <><div style={{ height: 1, background: 'rgba(255,255,255,0.03)', margin: '12px 0 8px' }} /><div style={{ fontFamily: 'var(--f-mono)', fontSize: 8, color: 'rgba(255,255,255,0.12)', textTransform: 'uppercase', letterSpacing: '0.1em', marginBottom: 6 }}>Used Substitutes</div>{subs.map((p: any, pi: number) => <PlayerRow key={pi} p={p} color={team.color} delay={0} />)}</>}</div>); })}</div>}
                </div>

                <div style={{ padding: `10px var(--cardPad)`, borderTop: '1px solid rgba(255,255,255,0.03)', display: 'flex', justifyContent: 'space-between', alignItems: 'center' }}>
