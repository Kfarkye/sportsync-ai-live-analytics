...
const HeaderCell = ({ label, align = 'center' }: { label: string, align?: 'left' | 'center' | 'right' }) => (
    <div className={`
      text-[9px] font-bold text-slate-400 uppercase tracking-[0.15em] py-2 px-3 border-r border-slate-100 last:border-r-0
...
const DenseCell = ({
    ...
    return (
        <div className={`
      relative flex flex-col justify-center h-[52px] px-3 border-r border-slate-100 last:border-r-0
      transition-colors duration-200 cursor-pointer group
      hover:bg-slate-50
      ${highlight ? 'bg-emerald-50' : ''}
    `}>
            {isEmpty ? (
                <span className="text-slate-300 text-xs font-mono text-center">-</span>
            ) : (
                <div className="flex flex-col gap-0.5">
                    <div className="flex justify-between items-baseline w-full">
                        <span className={`text-[13px] font-mono font-bold tracking-tighter ${highlight ? 'text-emerald-600' : 'text-slate-800 group-hover:text-slate-900'}`}>
                            {line}
                        </span>
                        {juice && (
                    ...
                    <div className="flex justify-between items-center w-full">
                        {label && <span className="text-[9px] font-bold text-slate-400 uppercase tracking-wider">{label}</span>}
                        {prob && prob !== '-' && (
                            <div className="flex items-center gap-1 ml-auto">
                                <span className="text-[9px] font-mono text-slate-400 group-hover:text-slate-600 tabular-nums">{prob}</span>
                            </div>
                        )}
                    </div>
                </div>
...
const BettingSnapshot: React.FC<BettingSnapshotProps> = ({ odds, homeTeam, awayTeam, sport }) => {
    ...
    return (
        <div className="bg-white rounded-xl border border-slate-200 overflow-hidden shadow-sm">
            ...
            {/* Compact Header */}
            <div className="grid grid-cols-[1.2fr_1fr_1fr_1fr] bg-slate-50 border-b border-slate-200">
            ...
            <div className="divide-y divide-slate-100">
                ...
                <div className="grid grid-cols-[1.2fr_1fr_1fr_1fr] items-stretch hover:bg-slate-50/50">
                    <div className="flex flex-col justify-center px-4 py-2 border-r border-slate-100">
                        <span className="font-bold text-slate-900 text-[13px] tracking-tight truncate">{awayName}</span>
                        <span className="text-[9px] text-slate-400 font-mono mt-0.5">AWAY</span>
                    </div>
                    <DenseCell
                ...
                {/* Draw Row (Soccer) */}
                {sport === Sport.SOCCER && (
                    <div className="grid grid-cols-[1.2fr_1fr_1fr_1fr] items-stretch bg-slate-50">
                        <div className="flex flex-col justify-center px-4 py-2 border-r border-slate-100">
                            <span className="font-bold text-slate-500 text-[13px] tracking-tight">Draw</span>
                        </div>
            ...
            {/* Footer */}
            <div className="bg-slate-50 border-t border-slate-200 px-3 py-1.5 flex justify-between items-center">
                <div className="flex items-center gap-2">
                    <div className="w-1 h-1 rounded-full bg-emerald-500 shadow-[0_0_5px_rgba(16,185,129,0.4)]" />
                    <span className="text-[9px] font-mono text-slate-400 uppercase tracking-widest">Real-time Feed</span>
                </div>
                <div className="flex items-center gap-3">
                    <span className="text-[9px] font-mono text-slate-400">IMP PROB %</span>
                    <button className="text-[9px] font-bold text-slate-500 hover:text-slate-900 flex items-center gap-1 transition-colors uppercase tracking-wider group">
                        Full Board
                        <ArrowUpRight className="w-2.5 h-2.5 text-slate-400 group-hover:text-slate-900 transition-colors" />
                    </button>
                </div>
            </div>
        </div>
