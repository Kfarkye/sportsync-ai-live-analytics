...
const DenseCell = ({ 
  ...
  return (
    <div className={`
      relative flex flex-col justify-center h-[52px] px-3 border-r border-edge last:border-r-0
      ...
      {isEmpty ? (
          ...
          <div className="flex flex-col gap-0.5">
              <div className="flex justify-between items-baseline w-full">
                  <span className={`text-body-sm font-mono font-bold tracking-tighter ${highlight ? 'text-[#53D337]' : 'text-zinc-200 group-hover:text-white'}`}>
                      {line}
                  </span>
                  {juice && (
              ...
              <div className="flex justify-between items-center w-full">
                  {label && <span className="text-label font-bold text-zinc-600 uppercase tracking-wider">{label}</span>}
                  {prob && prob !== '-' && (
                      <div className="flex items-center gap-1 ml-auto">
                          <span className="text-label font-mono text-zinc-600 group-hover:text-zinc-400 tabular-nums">{prob}</span>
                      </div>
                  )}
              </div>
          </div>
  ...
  return (
    <div className="bg-[#111113] rounded-xl border border-white/[0.1] overflow-hidden shadow-sm ring-1 ring-white/[0.05]">
        ...
        <div className="divide-y divide-white/[0.06]">
            ...
            {/* Away Row */}
            <div className="grid grid-cols-[1.2fr_1fr_1fr_1fr] items-stretch hover:bg-overlay-ghost">
                <div className="flex flex-col justify-center px-4 py-2 border-r border-edge">
                    <span className="font-bold text-white text-body-sm tracking-tight truncate">{awayName}</span>
                    <span className="text-label text-zinc-500 font-mono mt-0.5">AWAY</span>
                </div>
                <DenseCell 
            ...
            <div className="grid grid-cols-[1.2fr_1fr_1fr_1fr] items-stretch hover:bg-overlay-ghost">
                <div className="flex flex-col justify-center px-4 py-2 border-r border-edge">
                    <span className="font-bold text-white text-body-sm tracking-tight truncate">{homeName}</span>
                    <span className="text-label text-zinc-500 font-mono mt-0.5">HOME</span>
                </div>
                <DenseCell 
        ...
        <div className="bg-[#030303] border-t border-edge-subtle px-3 py-1.5 flex justify-between items-center">
            ...
            <div className="flex items-center gap-3">
                <span className="text-label font-mono text-zinc-700">IMP PROB %</span>
                <button className="text-label font-bold text-zinc-500 hover:text-white flex items-center gap-1 transition-colors uppercase tracking-wider group">
                    Full Board
