                ...
                return (
                    <div key={idx} className="space-y-8">
                        <div className="flex flex-col items-center">
                            <span className="text-[9px] font-black uppercase tracking-[0.5em] text-white/10 mb-2 truncate max-w-full">{category.displayName}</span>
                        </div>
                        ...
                        <div className="space-y-8">
                            ...
                            {awayLeader && (
                                <div className="flex items-center gap-4 group">
                                    <div className="relative w-10 h-10 rounded-full overflow-hidden bg-black/40 border border-white/[0.04]">
                                        {awayLeader.athlete.headshot ? (
                                            ...
                                            <div className="w-full h-full flex items-center justify-center text-[10px] text-zinc-800 font-bold uppercase">
                                                {(awayLeader.athlete.shortName || awayLeader.athlete.displayName || '').substring(0, 2)}
                                            </div>
                                        )}
                                    </div>
                                    <div className="flex-1 min-w-0">
                                        <div className="text-[12px] font-medium text-white/50 truncate group-hover:text-white transition-colors tracking-tight italic uppercase">{awayLeader.athlete.displayName}</div>
                            ...
                            {homeLeader && (
                                <div className="flex items-center gap-4 group flex-row-reverse text-right">
                                    ...
                                    <div className="flex-1 min-w-0">
                                        <div className="text-[12px] font-medium text-white/50 truncate group-hover:text-white transition-colors tracking-tight italic uppercase">{homeLeader.athlete.displayName}</div>
