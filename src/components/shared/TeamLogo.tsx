...
const TEAM_LOGO_LOADED_CACHE = new Set<string>();
...
const TeamLogo: React.FC<TeamLogoProps> = ({
    ...
    const fallbackColor = React.useMemo(() => teamColor || getTeamColor(name) || colorFromName(name), [name, teamColor]);
    const fallbackTextColor = React.useMemo(() => textColorForBackground(fallbackColor), [fallbackColor]);

    const logoSrcWithRetry = React.useMemo(() => {
        if (!optimizedSrc) return null;
        if (retryCount === 0) return optimizedSrc;
        const separator = optimizedSrc.includes('?') ? '&' : '?';
        return `${optimizedSrc}${separator}__rl=${retryCount}`;
    ...
    useEffect(() => {
        ...
        if (optimizedSrc && TEAM_LOGO_LOADED_CACHE.has(optimizedSrc)) {
            setLoaded(true);
        }
    }, [optimizedSrc]);

    const handleError = () => {
        if (!error && retryCount < 1 && optimizedSrc) {
    ...
    const handleLoad = () => {
        if (optimizedSrc) TEAM_LOGO_LOADED_CACHE.add(optimizedSrc);
        setLoaded(true);
        setError(false);
    };
    ...
    const hasValidSource = !!logoSrcWithRetry && !error;
            ...
            >
                {hasValidSource ? (
                    <>
                        ...
                        <img
                            src={logoSrcWithRetry || undefined}
                            alt={name}
                            className={cn(
    ...
    return (
        <div className={cn('relative overflow-hidden shrink-0', className)}>
            ...
            <img
                src={logoSrcWithRetry || undefined}
                alt={name}
                className={cn('w-full h-full object-contain transition-all duration-300', loaded ? 'opacity-100 scale-100' : 'opacity-0 scale-90')}
                onError={handleError}
                onLoad={handleLoad}
                loading="eager"
                decoding="sync"
