...
const SportAwareTensionWidget = ({
  ...
  const stateStyles = {
    ...
    'NEUTRAL': { accent: 'zinc', borderColor: 'border-slate-200' }
  };
  ...
  return (
    <div className={cn("relative overflow-hidden p-6 mb-8 bg-white/[0.01] border rounded-xl group", style.borderColor)}>
            ...
            <div className={cn(
              ...
              lean === 'OVER' ? "bg-emerald-500/10 border-emerald-500/20 text-emerald-400" :
                lean === 'UNDER' ? "bg-rose-500/10 border-rose-500/20 text-rose-400" :
                  "bg-zinc-500/10 border-zinc-500/20 text-slate-400"
            )}>
              <Activity size={12} />
              <span className="text-[10px] font-black uppercase tracking-widest">{lean}</span>
            </div>
          ) : (
            <div className="px-3 py-1.5 rounded-lg border bg-zinc-800/50 border-zinc-700 text-slate-500 flex items-center gap-2">
              <Scale size={12} />
      ...
      <div className="grid grid-cols-2 gap-4 relative z-10">
        <div className="p-4 bg-white/40 border border-white/5 rounded-lg flex flex-col gap-1">
          <span className="text-[8px] font-black uppercase tracking-widest text-slate-500 leading-none">Projected Total</span>
          <span className={cn("text-2xl font-mono-ledger font-black tabular-nums tracking-tighter",
            edgeState === 'NEUTRAL' ? "text-slate-400" : "text-emerald-400"
          )}>
            {blueprint?.model_number?.toFixed(1) || match.ai_signals?.deterministic_fair_total?.toFixed(1) || match.current_odds?.fairValue || '—'}
          </span>
        </div>
        <div className="p-4 bg-white/40 border border-white/5 rounded-lg flex flex-col gap-1">
          <span className="text-[8px] font-black uppercase tracking-widest text-slate-500 leading-none">Vegas Line</span>
          <span className="text-2xl font-mono-ledger font-black text-slate-900 tabular-nums tracking-tighter">
            {blueprint?.line?.toFixed(1) || match.current_odds?.total || match.odds?.total || match.odds?.overUnder || '—'}
          </span>
        </div>
      </div>

      <p className="mt-4 text-[10px] text-slate-500 leading-relaxed font-medium line-clamp-2">
        {edgeState === 'NEUTRAL'
...
const PPMTracker = ({ ppm, edgeState, edgePoints, context, sport }: {
  ...
  const deltaColor = ppm.delta > 0 ? "text-emerald-400" : ppm.delta < 0 ? "text-rose-400" : "text-slate-500";
    ...
    <div className={cn("mb-8 p-6 rounded-xl relative overflow-hidden border", config.bg, config.border)}>
      ...
      <div className="flex justify-between items-center mb-6">
        <div className="flex items-center gap-3">
          ...
          <div>
            ...
            <span className="text-[11px] font-bold text-slate-900 tracking-tight">Scoring Pace</span>
          </div>
        </div>
      ...
      <div className="grid grid-cols-3 gap-6 relative z-10 mb-4">
        <div className="flex flex-col gap-1">
          <span className="text-[8px] font-black uppercase tracking-widest text-slate-500">Current Pace</span>
          <span className="text-xl font-mono-ledger font-black text-slate-900 tabular-nums tracking-tighter">{ppm.observed.toFixed(3)}</span>
        </div>
        <div className="flex flex-col gap-1">
          <span className="text-[8px] font-black uppercase tracking-widest text-slate-500">Expected Pace</span>
          <span className="text-xl font-mono-ledger font-black text-slate-900 tabular-nums tracking-tighter opacity-70">{ppm.projected.toFixed(3)}</span>
        </div>
        <div className="flex flex-col gap-1">
          <span className="text-[8px] font-black uppercase tracking-widest text-slate-500">Pace Diff</span>
        ...
        <div className="flex items-center justify-between pt-4 border-t border-slate-200">
          <div className="flex items-center gap-4 text-[10px] font-mono text-slate-500">
            <span className="flex items-center gap-1.5">
              ...
              <span className="text-slate-500">P{context.period}</span>
            </span>
            <span className="text-slate-400">|</span>
            <span className="font-bold text-slate-400">{context.current_score}</span>
          </div>
          <div className="text-[9px] text-slate-500 font-mono">
            <span className="text-slate-500">{context.remaining_mins.toFixed(0)}m</span> remaining
          </div>
        </div>
      )}
  ...
  const fmtSpread = (val: string | number | null | undefined, price?: number) => {
    ...
    return (
      <div className="flex flex-col items-center leading-none group">
        ...
        {price && <span className="text-[10px] font-mono font-bold text-slate-500 mt-1 uppercase">{fmtVig(price)}</span>}
      </div>
    );
  };
  ...
  return (
    <div className="mb-24 rounded-sm overflow-hidden border border-slate-200 bg-white/20 shadow-[0_24px_48px_rgba(0,0,0,0.5)]">
      ...
      <div className="grid grid-cols-4 gap-px bg-slate-100 border-t border-slate-200">
        <div className="bg-white/40 p-12 flex items-center"><span className="text-[9px] font-black text-slate-900/40 uppercase tracking-[0.3em]">{s.label || 'Spread'}</span></div>
        <div className="bg-white/40 p-12 text-center flex flex-col justify-center">{fmtSpread(opening?.homeSpread || opening?.home_spread || opening?.spread, signals.odds?.open?.spreadPrice)}</div>
        <div className="bg-white/40 p-12 text-center flex flex-col justify-center opacity-60">{closing ? fmtSpread(closing?.homeSpread || closing?.home_spread || closing?.spread) : '—'}</div>
        <div className="bg-white/40 p-12 flex flex-col justify-center">
      ...
      <div className="grid grid-cols-4 gap-px bg-slate-100 border-t border-slate-200">
        <div className="bg-white/40 p-12 flex items-center"><span className="text-[9px] font-black text-slate-900/40 uppercase tracking-[0.3em]">Total</span></div>
        <div className="bg-white/40 p-12 text-center flex flex-col justify-center">{fmtTotal(opening?.overUnder || opening?.total || opening?.total_line, signals.odds?.open?.totalPrice)}</div>
        <div className="bg-white/40 p-12 text-center flex flex-col justify-center opacity-60">{closing ? fmtTotal(closing?.overUnder || closing?.total || closing?.total_line) : '—'}</div>
        <div className="bg-white/40 p-12 flex flex-col justify-center">
          <div className="flex items-center gap-4">
            <div className="w-1.5 h-1.5 rounded-full bg-emerald-500 shadow-[0_0_15px_rgba(16,185,129,0.8)] animate-pulse" />
            <div className="flex flex-col">
      ...
      <div className="grid grid-cols-4 gap-px bg-slate-100 border-t border-slate-200">
        <div className="bg-white/40 p-12 flex items-center"><span className="text-[9px] font-black text-slate-900/40 uppercase tracking-[0.3em]">Money</span></div>
        <div className="bg-white/40 p-12 text-center"><span className="text-4xl font-black text-slate-900/30 tracking-[-0.05em] tabular-nums">{fmtML(opening?.moneylineHome || opening?.homeWin || opening?.home_ml)}</span></div>
        <div className="bg-white/40 p-12 text-center"><span className="text-4xl font-black text-slate-900/20 tracking-[-0.05em] tabular-nums">{closing ? fmtML(closing?.moneylineHome || closing?.homeWin || closing?.home_ml) : '—'}</span></div>
        <div className="bg-white/40 p-12 flex flex-col justify-center">
          <div className="flex items-center gap-4">
            <div className="w-1.5 h-1.5 rounded-full bg-emerald-500 shadow-[0_0_15px_rgba(16,185,129,0.8)] animate-pulse" />
    ...
    <div className="mb-8 p-6 bg-slate-50 border border-slate-200 rounded-xl">
      ...
      <div className="grid grid-cols-2 lg:grid-cols-6 gap-6">
        <div className="space-y-2">
          <span className="text-[10px] font-semibold text-slate-500 uppercase tracking-wide block">Resistance</span>
          <div className="flex items-center gap-2">
            <div className={cn("w-1.5 h-1.5 rounded-full", constraints.sharp_resistance ? "bg-emerald-500" : "bg-zinc-700")} />
            <span className={cn("text-[11px] font-mono", constraints.sharp_resistance ? "text-emerald-400" : "text-slate-500")}>{constraints.sharp_resistance ? "Sharp" : "Stable"}</span>
          </div>
        </div>
        <div className="space-y-2">
          <span className="text-[10px] font-semibold text-slate-500 uppercase tracking-wide block">Public Shade</span>
          <div className="flex items-center gap-2">
            <div className={cn("w-1.5 h-1.5 rounded-full", constraints.public_shade ? "bg-amber-500" : "bg-zinc-700")} />
            <span className={cn("text-[11px] font-mono", constraints.public_shade ? "text-amber-400" : "text-slate-500")}>{constraints.public_shade ? "Active" : "None"}</span>
          </div>
        </div>
        <div className="space-y-2">
          <span className="text-[10px] font-semibold text-slate-500 uppercase tracking-wide block">Efficiency</span>
          <span className={cn("text-[12px] font-mono", (signals.efficiency_srs || 0) !== 0 ? "text-slate-900" : "text-slate-500")}>{(signals.efficiency_srs || 0) > 0 ? `+${signals.efficiency_srs!.toFixed(2)}` : (signals.efficiency_srs?.toFixed(2) || "0.00")}</span>
        </div>
        <div className="space-y-2">
          <span className="text-[10px] font-semibold text-slate-500 uppercase tracking-wide block">Stability</span>
          <span className="text-[12px] font-mono text-slate-400">{signals.efficiency_srs ? (1.0 / (1.0 + Math.abs(signals.efficiency_srs) / 1.5)).toFixed(3) : "1.000"}</span>
        </div>
        <div className="space-y-2">
          <span className="text-[10px] font-semibold text-slate-500 uppercase tracking-wide block">Adjustments</span>
          <span className={cn("text-[12px] font-mono", (signals.news_adjustment || 0) !== 0 ? "text-amber-400" : "text-slate-500")}>{(signals.news_adjustment || 0) > 0 ? `+${signals.news_adjustment!.toFixed(1)}` : (signals.news_adjustment?.toFixed(1) || "0.0")}</span>
        </div>
        <div className="space-y-2">
          <span className="text-[10px] font-semibold text-slate-500 uppercase tracking-wide block">Targeting</span>
          <div className="flex items-center gap-2">
            <div className={cn("w-1.5 h-1.5 rounded-full", signals.is_total_override ? "bg-emerald-500" : "bg-zinc-700")} />
            <span className={cn("text-[11px] font-mono", signals.is_total_override ? "text-emerald-400" : "text-slate-500")}>{signals.is_total_override ? (signals.override_classification === 'DELAY' ? "Priority" : "Structural") : "Standard"}</span>
          </div>
        </div>
      </div>
    </div>
...
const SharpOriginPill: React.FC<{ origin: SharpOrigin }> = ({ origin }) => {
  ...
  const accentColor = isActive ? 'text-emerald-400' : isPotential ? 'text-amber-400' : 'text-slate-500';
  ...
  return (
    <div className="flex flex-col gap-2 p-4 bg-slate-50 border border-slate-200 rounded-lg">
      <div className="flex items-center justify-between"><span className="text-[10px] font-semibold uppercase tracking-wide text-slate-500">{origin.label}</span><div className={cn("w-1.5 h-1.5 rounded-full", dotColor)} /></div>
      <span className={cn("text-[11px] font-mono", accentColor)}>{origin.status === 'NONE' ? 'Secure' : origin.value || origin.status}</span>
      <p className="text-[11px] text-slate-500 leading-relaxed line-clamp-2">{origin.description}</p>
    </div>
  );
};
...
export const LiveAnalysisCard: React.FC<{ match: Match }> = ({ match }) => {
  ...
  if (!analysis) {
    return (
      <div className="p-12 bg-white/40 backdrop-blur-3xl border border-white/5 rounded-sm flex flex-col items-center justify-center text-center">
        <span className="text-[11px] font-black uppercase tracking-[0.4em] text-slate-500 mb-6">Offline</span>
      </div>
    );
  }
    ...
    <div className={cn("p-8 relative overflow-hidden isolate", ESSENCE.card.base)}>
      ...
      <section className="mb-12 relative z-10">
        ...
        <div className="space-y-6">
          ...
          <div className="p-6 rounded-2xl bg-slate-50 border border-slate-200 backdrop-blur-sm">
