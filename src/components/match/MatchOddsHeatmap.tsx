    ...
    <div className="w-full space-y-3">
      ...
      <div className="flex flex-wrap items-center justify-between gap-2 rounded-xl border border-[#D9E2F3] bg-white px-3 py-2">
        <div className="inline-flex items-center gap-1 rounded-lg border border-slate-200 bg-slate-50 p-1">
          {(['all', 'main', 'props'] as OddsViewMode[]).map((mode) => (
            ...
            >
              {mode === 'all' ? 'All' : mode === 'main' ? 'Main' : 'Props'}
            </button>
          ))}
        </div>
      ...
      {isError ? (
        <div className="rounded-xl border border-rose-200 bg-rose-50 px-3 py-3 text-[11px] text-rose-700">
          Market depth feed unavailable right now.
        </div>
      ) : null}
      ...
      {hasRows ? (
        <div className="space-y-2.5">
          {allRows.map((row) => {
            ...
            return (
              ...
              >
                ...
                >
                  {expandedTicker === row.market_ticker ? 'Hide details' : 'Show details'}
                </button>
                ...
                {expandedTicker === row.market_ticker ? (
                  <div className="mt-2 grid grid-cols-3 gap-2 rounded-md border border-slate-200 bg-slate-50 px-2.5 py-2">
                    ...
                    <div>
                      <div className="text-[9px] uppercase tracking-[0.1em] text-slate-500">Open Interest</div>
          ...
          {!propsExpanded && orderedOddsRows.props.length > 0 ? (
            ...
            >
              Show player props ({orderedOddsRows.props.length})
            </button>
          ) : null}
          ...
          <div className="flex items-center justify-center gap-4 pt-1 text-[11px] text-slate-500">
            ...
            <span className="inline-flex items-center gap-1.5"><span className="inline-block h-2 w-3 rounded-[999px] bg-[#E24B4A]" />Split</span>
        ...
        <div className="rounded-xl border border-slate-200 bg-white px-3 py-3 text-[11px] text-slate-500">
          Market depth is not available for this matchup yet.
        </div>
      )}
    </div>
  );
