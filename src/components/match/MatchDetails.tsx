...
const MatchDetails: FC<MatchDetailsProps> = ({ match: initialMatch, onBack, matches = [], onSelectMatch }) => {
  ...
  const TABS = useMemo(() => {
    if (isSched) return [{ id: "DETAILS", label: "Matchup" }, { id: "PROPS", label: "Props" }, { id: "DATA", label: "Analysis" }];
    const live = [{ id: "OVERVIEW", label: "Game" }, { id: "PROPS", label: "Props" }, { id: "DATA", label: "Analysis" }];
          ...
          <LayoutGroup>
            <AnimatePresence mode="wait">
              ...
              <motion.div key={deferredTab} {...PHYSICS.SLIDE_UP} className="transform-gpu will-change-transform">
                ...
                {deferredTab === 'PLAYS' && isBaseball && (
                  <div className="space-y-4">
                    <SpecSheetRow label="01 // LINE SCORE" defaultOpen={true} collapsible={false}>
                      <BaseballLineScore
                        match={match}
                        currentInning={match.period ?? undefined}
                        inningHalf={baseballData?.inningHalf}
                      />
                    </SpecSheetRow>
                    {baseballData?.scoringPlays && baseballData.scoringPlays.length > 0 && (
                      <SpecSheetRow label="02 // SCORING PLAYS" defaultOpen={true} collapsible={false}>
                        <BaseballScoringSummary
                ...
                {deferredTab === 'DATA' && (
                  <div className="space-y-0">
                    ...
                    {/* Evidence / Authority / Health Metrics */}
                    <SpecSheetRow label="08 // EVIDENCE LOG" defaultOpen={false}>
                      <TechnicalAuditHub canonicalId={getDbMatchId(match.id, match.leagueId?.toLowerCase() || '')} />
                    </SpecSheetRow>
                  </div>
                )}
