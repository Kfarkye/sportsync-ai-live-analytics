...
const TacticalHUD: FC<{ content: string }> = memo(({ content }) => (
  <motion.div layout initial={{ x: -5, opacity: 0 }} animate={{ x: 0, opacity: 1 }} transition={SYSTEM.anim.fluid}
    className={cn("my-6 relative overflow-hidden rounded-r-[16px] border-l-[3px] border-l-amber-500/80 shadow-[20px_0_40px_-10px_rgba(245,158,11,0.05)]", SYSTEM.surface.hud)}>
    <div className="p-5">
...
const ThinkingPill: FC<{ onStop?: () => void; status?: string }> = memo(({ onStop, status = 'thinking' }) => {
  ...
  return (
    <motion.div layout initial={{ opacity: 0, y: 10, scale: 0.95 }} animate={{ opacity: 1, y: 0, scale: 1 }} exit={{ opacity: 0, y: 10, scale: 0.95 }} transition={SYSTEM.anim.fluid}
      ...
      <OrbitalRadar />
      <AnimatePresence mode="wait"><motion.span key={text} initial={{ opacity: 0, filter: 'blur(4px)' }} animate={{ opacity: 1, filter: 'blur(0px)' }} exit={{ opacity: 0, filter: 'blur(4px)' }} className={cn(SYSTEM.type.mono, 'text-zinc-300 min-w-[100px] text-center')}>{text}</motion.span></AnimatePresence>
    ...
    <div className="flex gap-2 overflow-x-auto pb-3 scrollbar-hide px-6">
      {chips.map((c, i) => (
        <motion.button key={c} onClick={() => { triggerHaptic(); onSelect(q[c]); }}
          ...
          <span className="text-[10px] font-medium text-zinc-300 tracking-wide uppercase">{c}</span>
        </motion.button>
      ))}
    </div>
  );
        ...
        <div className={cn("prose prose-invert max-w-none", isUser && "prose-p:text-black/90")}>
          <ReactMarkdown remarkPlugins={[remarkGfm]} components={{
            p: ({ children }) => {
              ...
              return <p className={cn(SYSTEM.type.body, isUser ? "text-[#1a1a1a]" : "text-[#A1A1AA]", "mb-5 last:mb-0")}>{children}</p>;
            },
            strong: ({ children }) => {
              const t = String(children);
              if (['Analytical Walkthrough', 'Market Dynamics', 'Sentiment Signal', 'Structural Assessment'].some(k => t.includes(k))) {
                return <div className="mt-8 mb-3 flex items-center gap-2 pb-2 border-b border-white/5"><Activity size={12} className="text-emerald-500" /><span className="text-[10px] font-mono text-emerald-500 uppercase tracking-widest">{children}</span></div>;
              }
...
const InputDeck: FC<{ value: string; onChange: (v: string) => void; onSend: () => void; onStop: () => void; attachments: Attachment[]; onAttach: (a: Attachment[]) => void; isProcessing: boolean; isVoiceMode: boolean; onVoiceModeChange: (v: boolean) => void; inputRef: RefObject<HTMLTextAreaElement | null>; fileInputRef: RefObject<HTMLInputElement | null>; }> = memo(({ value, onChange, onSend, onStop, attachments, onAttach, isProcessing, isVoiceMode, onVoiceModeChange, inputRef, fileInputRef }) => {
  ...
  const handleFileChange = (e: ChangeEvent<HTMLInputElement>) => {
    if (e.target.files?.[0]) {
      ...
      r.onload = async () => onAttach([...attachments, { file: f, base64: (r.result as string).split(',')[1], mimeType: f.type }]);
      r.readAsDataURL(f);
    }
    e.target.value = '';
  };
  ...
  return (
    <motion.div layout className={cn('flex flex-col gap-2 p-1.5 relative overflow-hidden transition-colors duration-500', SYSTEM.geo.input, 'bg-[#0A0A0B] shadow-2xl', isVoiceMode ? 'border-emerald-500/30 shadow-[0_0_40px_-10px_rgba(16,185,129,0.15)]' : SYSTEM.surface.milled)} transition={SYSTEM.anim.fluid}>
      ...
      <div className="flex items-end gap-2">
        ...
        {isVoiceMode ? (
          <div className="flex-1 flex items-center justify-center h-[52px] gap-3"><OrbitalRadar /><span className={cn(SYSTEM.type.mono, 'text-emerald-500 tracking-widest')}>LISTENING</span></div>
        ) : (
        ...
        <div className="flex items-center gap-1 pb-1.5 pr-1">
          {!value && !attachments.length && <button onClick={toggleVoice} className={cn('p-3 rounded-[18px]', isVoiceMode ? 'text-rose-400 bg-rose-500/10' : 'text-zinc-500 hover:bg-white/5 hover:text-white transition-colors')}>{isVoiceMode ? <MicOff size={18} /> : <Mic size={18} />}</button>}
          {(value || attachments.length > 0 || isProcessing) && <motion.button initial={{ scale: 0.9 }} animate={{ scale: 1 }} whileTap={{ scale: 0.92 }} onClick={() => isProcessing ? onStop() : onSend()} className={cn('p-3 rounded-[18px] transition-all duration-300', (value || attachments.length || isProcessing) ? 'bg-white text-black shadow-[0_0_15px_rgba(255,255,255,0.2)]' : 'bg-white/5 text-zinc-600')}>{isProcessing ? <StopCircle size={18} className="animate-pulse" /> : <ArrowUp size={18} strokeWidth={2.5} />}</motion.button>}
        </div>
      </div>
    </motion.div>
  );
