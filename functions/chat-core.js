/* ============================================================================
   functions/chat-core.js
   "Obsidian Citadel" — Production Backend (v29.0 Titanium Edition)

   Architecture: Firebase Cloud Functions (v2) + Express-style req/res
   ...
   ├─ waitUntil() → Background work runs inline (CF doesn't terminate until all promises resolve)
   ├─ Web API Request/Response → Express (req, res) via onRequest
   ├─ ReadableStream → res.write() / res.end() for SSE streaming
   ├─ req.signal.aborted → AbortController managed via req.on('close')
   └─ Vercel env vars → Firebase secrets + defineString()
============================================================================ */
...
// waitUntil removed — Firebase Cloud Functions run until all async work completes
...
// 🛡️ LAZY INITIALIZATION: Firebase secrets are injected at container startup but
// may not be available during ESM module evaluation. Defer client creation to first call.
let genAI = null;
let supabase = null;
...
// Bounded local memory cache survives Firebase Cloud Functions warm instances
...
function getPublicOrigin() {
    // Firebase Hosting canonical URL
    if (process.env.FIREBASE_HOSTING_URL) return process.env.FIREBASE_HOSTING_URL;
    return "https://sportsync-evidence.web.app";
}
...
// Firebase/Express-compatible header accessor (works with both Web API Headers and plain objects)
const getHeader = (headers, key) => {
    if (typeof headers?.get === 'function') return headers.get(key);
    if (headers && typeof headers === 'object') return headers[key] || headers[key.toLowerCase()] || null;
    return null;
};

const getClientIp = (headers) => {
    // 🛡️ SEC: Cloud Functions injects x-forwarded-for via Google Front End
    const candidates = [
...
// =============================================================================
// FIREBASE CLOUD FUNCTIONS HANDLER (Express-style req/res with SSE streaming)
// =============================================================================

export async function handleChat(req, res) {
    // 🛡️ INFRA: Lazy-init clients (Firebase secrets available at runtime, not module load)
    ensureInitialized();

    // 🛡️ SEC: Required env validation w/ explicit 500 response
    if (!genAI || !supabase) {
        res.status(500).json({ error: "Server misconfigured: missing required env vars." });
        return;
    }
    ...
    // Firebase Cloud Functions auto-parse JSON body via bodyParser
    let body;
    ...
    // Background quality guard log (no waitUntil needed — Firebase CF runs until all promises resolve)
    const qualityLogPromise = logResponseQualityGuard({
    ...
    // -------------------------------------------------------------------------
    // STREAM ENGINE: Express SSE via res.write()
    // -------------------------------------------------------------------------
    ...
    // AbortController to replace req.signal from Web API
    ...
    try {
        ...
        const result = await genAI.models.generateContentStream({
            ...
            abortSignal: abortController.signal // 🌟 Kills Google billing immediately if connection closes
        });

        for await (const chunk of result) {
            if (!streamActive || abortController.signal.aborted) break;
        ...
        // 🌟 Background Database Execution (inline — no waitUntil needed) 🌟
        if (streamActive && !abortController.signal.aborted && fullText) {
            try {
                const dbTasks = [];
        ...
        if (e?.name === "AbortError" || abortController.signal.aborted || !streamActive) {
