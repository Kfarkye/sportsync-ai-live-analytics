...
// ── Live Page Data (combined odds + props for live score page) ────────────────
...
function formatBookName(key) {
  const map = {
    ...
    caesars: 'Caesars', pointsbet: 'PointsBet', betonlineag: 'BetOnline',
    bovada: 'Bovada', wynnbet: 'WynnBet', betrivers: 'BetRivers',
    unibet_us: 'Unibet', espnbet: 'ESPN BET', pinnacle: 'Pinnacle',
    mybookieag: 'MyBookie', superbook: 'SuperBook', williamhill_us: 'Caesars',
  };
  return map[key] || key?.replace(/_/g, ' ').replace(/\b\w/g, c => c.toUpperCase()) || 'Unknown';
}

export const livePageData = onRequest(
  {
    region: 'us-central1',
    timeoutSeconds: 30,
    memory: '256MiB',
  ...
  async (req, res) => {
    ...
    try {
      ...
      // Fetch market feeds for multi-book odds
      let bookmakers = [];
      if (match.start_time) {
        ...
        const feedRes = await supabase
          .from('market_feeds')
          .select('raw_bookmakers, best_h2h, best_spread, best_total, home_team, away_team, canonical_id')
        ...
        if (feed) {
          try {
            const raw = typeof feed.raw_bookmakers === 'string'
              ? JSON.parse(feed.raw_bookmakers) : feed.raw_bookmakers;
          ...
          } catch { /* parse failed, use best_ fields */ }
      ...
      // Group props by player (over side only)
      const propsMap = {};
      for (const p of (propsRes.data || [])) {
        if (p.side !== 'over') continue;
        const key = p.player_name;
        if (!propsMap[key]) {
          propsMap[key] = { name: p.player_name, team: p.team, headshot: p.headshot_url, markets: [] };
        }
      ...
      return res.json({
        ...
        home: { team: match.home_team, team_id: match.home_team_id, score: match.home_score, ml: match.odds_home_ml_safe },
        away: { team: match.away_team, team_id: match.away_team_id, score: match.away_score, ml: match.odds_away_ml_safe },
        odds: { spread: match.odds_home_spread_safe, total: match.odds_total_safe, current: match.current_odds, opening: match.opening_odds, win_probability: match.win_probability },
        bookmakers: bookmakers.slice(0, 8),
        props: Object.values(propsMap),
        start_time: match.start_time,
      });
    } catch (e) {
      logger.error('[Live Page Data]', e.message);
