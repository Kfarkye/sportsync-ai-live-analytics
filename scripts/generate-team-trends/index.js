...
function renderIndexPage(teamSummaries) {
  const rows = teamSummaries
    ...
    .map(s => {
      ...
      const homeAts = `${s.home.covers}\u2013${s.home.nonCovers}`;
      const homeOverClass = s.home.overPct >= 55 ? 'hot' : s.home.overPct <= 45 ? 'cold' : '';
      const homeAtsClass = s.home.coverPct >= 55 ? 'hot' : s.home.coverPct <= 45 ? 'cold' : '';
      const awayOverClass = s.away.overPct >= 55 ? 'hot' : s.away.overPct <= 45 ? 'cold' : '';
      ...
      return `        <tr>
          ...
          <td class="record-cell ${homeAtsClass}">${homeAts} <span class="record-pct">(${s.home.coverPct}%)</span></td>
