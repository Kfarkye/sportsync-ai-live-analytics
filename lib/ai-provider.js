...
const googleClient = {
  ...
  async chatStreamRaw(contents, req) {
    ...
    // Diagnostic: verify tools + toolConfig reach the API
    log.info("chatStreamRaw_request", {
      model: req.model,
      hasTools,
      toolNames: toolObj.functionDeclarations?.map(f => f.name) || [],
      toolConfigMode: body.toolConfig?.functionCallingConfig?.mode || "NONE",
      hasSystemInstruction: !!body.systemInstruction,
      thinkingLevel: body.generationConfig?.thinkingConfig?.thinkingLevel || "NONE",
...
function parseGeminiSSEPayload(parsed) {
  ...
  // Diagnostic: log part types to verify function calls reach the parser
  if (parts.length > 0) {
    const partTypes = parts.map(p => p.functionCall ? `functionCall:${p.functionCall.name}` : p.thought ? "thought" : p.text !== undefined ? "text" : Object.keys(p).join(","));
    log.info("sse_parts", { types: partTypes, finishReason: candidate.finishReason || "none" });
  }
  const textParts = [];
  const thoughtParts = [];
  const functionCalls = [];
