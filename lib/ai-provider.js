...
class CircuitBreakerManager {
  ...
  recordSuccess(provider) {
    ...
    this.lockoutDuration[provider] = 6e4;
  ...
  recordFailure(provider, errorType) {
    ...
    if (errorType === "quota_exhausted") {
      ...
      this.lockoutDuration[provider] = 3e5;
    } else {
      this.lockoutDuration[provider] = 6e4;
    }
  ...
  isOpen(provider) {
    ...
    const lockout = this.lockoutDuration[provider] ?? 6e4;
  ...
  getStatus() {
    ...
    for (const p of ["google", "openai", "anthropic"]) {
      if ((this.failures[p] ?? 0) < 3) s[p] = "closed";
        ...
        const lockout = this.lockoutDuration[p] ?? 6e4;
    ...
    try {
      ...
      if (res.status === 429) {
        ...
        try {
          ...
          isHardQuota = status === "RESOURCE_EXHAUSTED" || message.includes("insufficient_quota") || message.includes("exceeded your current quota");
        } catch {
          isHardQuota = errorText.includes("RESOURCE_EXHAUSTED") || errorText.includes("insufficient_quota") || errorText.includes("exceeded your current quota");
        }
        if (isHardQuota) {
...
const googleClient = {
  ...
  async chatStreamRaw(contents, req) {
    ...
    const url = `https://generativelanguage.googleapis.com/v1beta/models/${req.model}:streamGenerateContent?alt=sse`;
    const res = await fetchWithRetry(url, { method: "POST", headers: { "Content-Type": "application/json", "x-goog-api-key": apiKey }, body: JSON.stringify(body), signal: req.signal }, 3e4, req.retries ?? 1, "google");
    if (!res.ok) throw new ProviderError("google", `${res.status}: ${await readErrorBody(res)}`, classifyHttpError(res.status));
    if (!res.body) throw new ProviderError("google", "Empty body", "server");
    return res.body;
  }
...
function mapErrorTypeToSelectionReason(errorType) {
  switch (errorType) {
