...
function resolveOddsSportKey(match) {
...
function extractHistoricalSnapshot(payload, oddsApiEventId, homeTeam, awayTeam, capturedAt) {
  const root = payload;
  const events = Array.isArray(root?.data) ? root.data : Array.isArray(payload) ? payload : [];
  if (!events.length) return null;
  const homeNorm = normalizeTeamName(homeTeam);
  const awayNorm = normalizeTeamName(awayTeam);
  let event = oddsApiEventId ? events.find((e) => typeof e.id === "string" && e.id === oddsApiEventId) : void 0;
  ...
  const book = preferredBooks.map((key) => bookmakers.find((b) => b.key === key)).find(Boolean) || bookmakers[0];
...
function hasOpeningLine(signal) {
  ...
  get_live_odds: async (args, ctx) => {
    ...
    try {
      ...
      const [matchRes, liveRes, openingRes] = await Promise.all([
        ctx.supabase.from("matches").select("id, home_team, away_team, current_odds, status, start_time, sport, league_id, odds_api_event_id").eq("id", matchId).maybeSingle(),
        ctx.supabase.from("live_game_state").select("odds, t0_snapshot, t60_snapshot").eq("id", matchId).maybeSingle(),
        ctx.supabase.from("opening_lines").select("home_spread, away_spread, total, home_ml, away_ml, provider, created_at").eq("match_id", matchId).maybeSingle()
      ]);
      ...
      const openingFromSnapshot = liveState?.t0_snapshot?.odds || {};
      const openingFromTable = openingRes.data || {};
      ...
      let openingSignal = {
        ...
        captured_at: typeof openingFromTable.created_at === "string" ? openingFromTable.created_at : null,
        bookmaker: typeof openingFromTable.provider === "string" ? openingFromTable.provider : null
      };
      ...
      if (!hasOpeningLine(openingSignal) && match.start_time) {
        ...
        if (Number.isFinite(kickoffMs)) {
          ...
          for (const offset of probeOffsetsHours) {
            ...
            const probeDate = new Date(kickoffMs - offset * 60 * 60 * 1e3).toISOString();
            try {
      ...
      const spreadDelta = currentHomeSpread !== null && openingSignal.home_spread !== null ? Number((currentHomeSpread - openingSignal.home_spread).toFixed(2)) : null;
      const totalDelta = currentTotal !== null && openingSignal.total !== null ? Number((currentTotal - openingSignal.total).toFixed(2)) : null;
      const homeMlDelta = currentHomeML !== null && openingSignal.home_ml !== null ? Number((currentHomeML - openingSignal.home_ml).toFixed(0)) : null;
      return {
        success: true,
        data: {
          ...
          line_notes: hasOpeningLine(openingSignal) ? "Opening and current lines are sourced from captured market data." : "Opening line unavailable from opening_lines, t0 snapshot, and Odds API historical probes.",
