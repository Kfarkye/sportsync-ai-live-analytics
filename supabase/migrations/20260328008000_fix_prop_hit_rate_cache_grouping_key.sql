-- Align prop_hit_rate_cache aggregation key with idx_phrc_unique.
-- The cache unique key does not include espn_player_id, so aggregating by espn_player_id
-- can generate duplicate inserts for the same cache key.
...
BEGIN
  ...
  agg AS (
    ...
    FROM expanded e
    GROUP BY e.player_name, e.bet_type, e.line_bucket, e.context_key, e.context_value
  )
