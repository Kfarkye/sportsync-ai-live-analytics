-- Follow-up MLB grading hardening:
-- 1) keep name-normalization improvements from prior migration
-- 2) coerce batter_total_bases to 0 when a matched batter has AB=0 and H=0
...
BEGIN
  ...
  resolved AS (
    SELECT
      ...
      COALESCE(NULLIF(bp.player_name, ''), bm.athlete_name, pm.athlete_name) AS out_player_name,
      ...
      CASE
        ...
        WHEN bp.bet_type = 'batter_total_bases' THEN COALESCE(
          bm.total_bases::numeric,
          CASE
            WHEN COALESCE(bm.at_bats, 0) = 0 AND COALESCE(bm.hits, 0) = 0 THEN 0::numeric
            ELSE NULL
          END
        )
        ELSE NULL
