-- Migration: bridge_soccer_live_odds_into_market_context
-- Keep live_odds_snapshots as the primary market context source and bridge
-- soccer_live_odds_snapshots as a fallback for soccer matches where unified
-- odds snapshots are absent or sparse.
...
WITH joined AS (
  SELECT
    ...
    COALESCE(los.id, ABS(hashtext(slos.id))::bigint) AS odds_snapshot_id,
    COALESCE(los.captured_at, slos.captured_at) AS odds_captured_at,
    CASE
      WHEN COALESCE(los.captured_at, slos.captured_at) IS NULL THEN NULL
      ELSE ABS(EXTRACT(EPOCH FROM (COALESCE(los.captured_at, slos.captured_at) - e.event_at)))::integer
    END AS odds_time_delta_sec,
    COALESCE(los.provider, 'SoccerLiveSnapshot') AS odds_provider,
    COALESCE(los.provider_id, slos.source) AS odds_provider_id,
    COALESCE(los.market_type, 'main') AS odds_market_type,
    COALESCE(los.status, 'live') AS odds_status,
    COALESCE(los.home_ml, slos.live_home_ml::integer) AS home_ml,
    COALESCE(los.away_ml, slos.live_away_ml::integer) AS away_ml,
    COALESCE(los.draw_ml, slos.live_draw_ml::integer) AS draw_ml,
    COALESCE(los.spread_home, slos.live_spread) AS spread_home,
    COALESCE(los.spread_away, CASE WHEN slos.live_spread IS NOT NULL THEN -slos.live_spread ELSE NULL END) AS spread_away,
    COALESCE(los.spread_home_price, slos.live_home_spread_price::integer) AS spread_home_price,
    COALESCE(los.spread_away_price, slos.live_away_spread_price::integer) AS spread_away_price,
    COALESCE(los.total, slos.live_total) AS total,
    COALESCE(los.over_price, slos.live_over_price::integer) AS over_price,
    COALESCE(los.under_price, slos.live_under_price::integer) AS under_price,
    COALESCE(los.is_live, true) AS odds_is_live,
    COALESCE(los.source, slos.source, 'soccer_live_odds_snapshots') AS odds_source,
    COALESCE(
      los.raw_payload,
      jsonb_build_object(
  ...
  LEFT JOIN LATERAL (
    ...
    WHERE e.sport = 'soccer'
      AND slos.match_id = e.match_id
      AND slos.captured_at BETWEEN e.event_at - INTERVAL '10 minutes' AND e.event_at + INTERVAL '10 minutes'
    ORDER BY
      ABS(EXTRACT(EPOCH FROM (slos.captured_at - e.event_at))) ASC,
      slos.captured_at DESC
    LIMIT 1
  ) slos
    ON true
),
