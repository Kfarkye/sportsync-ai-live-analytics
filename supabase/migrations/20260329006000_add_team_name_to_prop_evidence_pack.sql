-- Extend generate_prop_evidence_pack() to emit team_name for each card so
-- /props can render team context directly under player names.
...
BEGIN
  ...
  alias_team_ranked AS (
    SELECT
      CASE
        WHEN NULLIF(trim(a.espn_player_id::text), '') IS NOT NULL THEN 'id:' || NULLIF(trim(a.espn_player_id::text), '')
        ELSE 'name:' || public.norm_name_key(coalesce(a.canonical_player_name, a.alias_name))
      END AS player_key,
      ...
      row_number() OVER (
        PARTITION BY
          CASE
            ...
            ELSE 'name:' || public.norm_name_key(coalesce(a.canonical_player_name, a.alias_name))
          END
        ORDER BY coalesce(a.sample_count, 0) DESC, a.updated_at DESC NULLS LAST, a.team_name
      ) AS rn
    FROM public.player_prop_identity_aliases a
    WHERE lower(coalesce(a.league_id, '')) = 'nba'
      AND NULLIF(trim(a.team_name), '') IS NOT NULL
  ),
  alias_team_choice AS (
    SELECT
      atr.player_key,
      atr.team_name
    FROM alias_team_ranked atr
    WHERE atr.rn = 1
  ),
  outcome_team_ranked AS (
    SELECT
      CASE
        WHEN NULLIF(trim(o.espn_player_id::text), '') IS NOT NULL THEN 'id:' || NULLIF(trim(o.espn_player_id::text), '')
        ELSE 'name:' || public.norm_name_key(o.player_name)
      END AS player_key,
      NULLIF(trim(o.team), '') AS team_name,
      row_number() OVER (
        PARTITION BY
          CASE
            WHEN NULLIF(trim(o.espn_player_id::text), '') IS NOT NULL THEN 'id:' || NULLIF(trim(o.espn_player_id::text), '')
            ELSE 'name:' || public.norm_name_key(o.player_name)
          END
        ORDER BY o.game_date DESC NULLS LAST, o.team
      ) AS rn
    FROM public.player_prop_outcomes o
    WHERE lower(coalesce(o.league_id, '')) = 'nba'
      AND lower(coalesce(o.bet_type, '')) = ANY (v_markets)
      AND NULLIF(trim(o.team), '') IS NOT NULL
  ),
  outcome_team_choice AS (
    SELECT
      otr.player_key,
      otr.team_name
    FROM outcome_team_ranked otr
    WHERE otr.rn = 1
  ),
  cards AS (
    SELECT
      jsonb_build_object(
        ...
        'team_name', coalesce(atc.team_name, otc.team_name),
        'market', ba.market,
        'direction', CASE
    ...
    LEFT JOIN alias_team_choice atc
      ON atc.player_key = ba.player_key
    LEFT JOIN outcome_team_choice otc
      ON otc.player_key = ba.player_key
