-- Fix MLB prop outcome matching residual pending rows:
-- 1) rank batter/pitcher candidates per prop row (prop_bet_id), not per player bucket
-- 2) add short-name fallback: first initial + last name + team-context guard
...
BEGIN
  ...
  WITH base_props AS (
    SELECT
      ...
      pb.team,
      pb.opponent,
      regexp_replace(lower(coalesce(pb.player_name, '')), '[^a-z0-9]', '', 'g') AS norm_player_name,
      left(regexp_replace(lower(split_part(coalesce(pb.player_name, ''), ' ', 1)), '[^a-z0-9]', '', 'g'), 1) AS norm_first_initial,
      regexp_replace(
        lower(
          coalesce(
            (regexp_split_to_array(trim(coalesce(pb.player_name, '')), '\s+'))[
              array_length(regexp_split_to_array(trim(coalesce(pb.player_name, '')), '\s+'), 1)
            ],
            ''
          )
        ),
  ...
  batter_logs_norm AS (
    SELECT
      bgl.*,
      regexp_replace(lower(coalesce(bgl.athlete_name, '')), '[^a-z0-9]', '', 'g') AS norm_player_name,
      left(regexp_replace(lower(split_part(coalesce(bgl.athlete_name, ''), ' ', 1)), '[^a-z0-9]', '', 'g'), 1) AS norm_first_initial,
      regexp_replace(
        lower(
          coalesce(
  ...
  pitcher_logs_norm AS (
    SELECT
      pgl.*,
      regexp_replace(lower(coalesce(pgl.athlete_name, '')), '[^a-z0-9]', '', 'g') AS norm_player_name,
      left(regexp_replace(lower(split_part(coalesce(pgl.athlete_name, ''), ' ', 1)), '[^a-z0-9]', '', 'g'), 1) AS norm_first_initial,
      regexp_replace(
        lower(
          coalesce(
  ...
  batter_match AS (
    ...
    JOIN batter_logs_norm bgl
     ...
     AND (
       ...
       OR (
         bp.espn_player_id IS NULL
         AND bp.norm_last_name <> ''
...
-- Keep grants stable for pipeline invocations.
