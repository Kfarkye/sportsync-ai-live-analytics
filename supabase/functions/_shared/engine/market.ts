...
import { Sport, ExtendedMatch, OddsSnapshot, AISignals, EdgeEnvironmentTag } from "../types.ts";
import { SYSTEM_GATES } from "../gates.ts";
import { isFootball, clamp } from "./utils.ts";
import { getOddsValue } from "../oddsUtils.ts";

export function getCanonicalOdds(match: any): OddsSnapshot {
    ...
    return {
        ...
        cur: {
            spread: parse(c.spread ?? c.homeSpread, "spread") ?? parse(o.homeSpread ?? o.spread, "spread") ?? 0,
            total: parse(c.total ?? c.overUnder, "total") ?? parse(o.overUnder, "total") ?? 0,
...
export function calculateLiabilityInertia(match: ExtendedMatch, odds: OddsSnapshot) {
    ...
    const pubBetPct = (match as any).public_betting_pct || 50;
...
export function calculateEdgeEnvironment(match: any, odds: OddsSnapshot, progress: number) {
...
export function calculateNFLTotalOverride(match: any, _odds: OddsSnapshot, progress: number, srs: number) {
...
export const getMarketBlueprint = (match: any, signals: Partial<AISignals>): any => {
    const odds = (signals as any).odds;
    ...
    const direction = (signals.narrative as any)?.market_lean || 'PASS';
    ...
    return {
        ...
        status: (signals.narrative as any)?.signal_label || 'LIVE_READ',
        is_valid: isValid,
