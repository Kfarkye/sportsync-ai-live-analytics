...
export const EspnAdapters = {
    ...
    Situation: (data: any): Situation | undefined => {
        ...
        const result: Situation = {
            ...
            // NBA Spatial Discovery (v7.2)
            ballX: Safe.number(situationData.coordinate?.x ?? situationData.location?.x),
            ballY: Safe.number(situationData.coordinate?.y ?? situationData.location?.y),
            playerId: Safe.string(situationData.athlete?.id || situationData.lastPlay?.athleteId),
        };

        // Surgical logging for NBA/Basketball spatial audit
        const sport = data.header?.sport?.name || data.sport;
        if (sport?.toLowerCase().includes('basketball')) {
            console.log(`[NBA-Audit] Situation Data:`, JSON.stringify({
                possession: result.possessionId,
                ballX: result.ballX,
                ballY: result.ballY,
                playerId: result.playerId,
                raw_coordinate: situationData.coordinate || situationData.location
            }));
        }
    ...
    LastPlay: (data: any): LastPlay | undefined => {
        // PRIORITY 1: Full text from current drive plays (includes formations like "(Shotgun)")
        const drivePlay = data.drives?.current?.plays?.slice(-1)[0];

        // PRIORITY 2: Fallback to situation/lastPlay (may be abbreviated)
        let lastPlayData = drivePlay || data.lastPlay || data.situation?.lastPlay;
        if (!lastPlayData && Array.isArray(data.plays) && data.plays.length > 0) {
            lastPlayData = data.plays[data.plays.length - 1];
        }
        if (!lastPlayData) return undefined;

        // Use the text directly - drive plays have full text, situation may be abbreviated
        const playText = Safe.string(lastPlayData.text) || '';
    ...
    // 🆕 RecentPlays: Extract last 5 plays for richer AI context
    RecentPlays: (data: any): Array<{ text: string; clock: string; type: string; period: number; team?: string }> => {
        // Try multiple ESPN data paths for plays
        ...
        // Take last 5 plays and format for AI consumption
        return plays.slice(-5).reverse().map((play: any) => ({
            text: Safe.string(play.text) || Safe.string(play.description) || 'Play data unavailable',
            clock: Safe.string(play.clock?.displayValue) || Safe.string(play.wallclock) || '',
            type: Safe.string(play.type?.text) || Safe.string(play.type?.abbreviation) || 'unknown',
            period: Safe.number(play.period?.number, 1),
            team: Safe.string(play.team?.displayName || play.team?.shortDisplayName)
        })).filter((p: any) => p.text && p.text !== 'Play data unavailable');
    }
};
