...
function extractPostgame(data: any, eventId: string, leagueKey: string, matchId: string, startTime: string): any {
...
Deno.serve(async (req: Request) => {
  ...
  const leagueParam = url.searchParams.get('league') || 'epl';
  ...
  for (const leagueKey of requestedLeagues) {
    ...
    try {
      ...
      const { data: matchRows, error: matchErr } = await supabase.from('matches').select('id, home_team, away_team, start_time, status, league_id').eq('league_id', cfg.espn).eq('status', 'STATUS_FINAL').gte('start_time', cutoff.toISOString()).order('start_time', { ascending: false });
      ...
      const drainResults = await processBatch(toDrain, MAX_CONCURRENT, INTER_BATCH_MS, async (row: any) => {
        ...
        try {
          const res = await fetchWithTimeout(summaryUrl, FETCH_TIMEOUT_MS);
          ...
          return extractPostgame(data, eventId, leagueKey, row.id, row.start_time);
