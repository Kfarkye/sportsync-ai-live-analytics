...
async function drainTeamStats(supabase: any, leagueKey: string, league: LeagueDef): Promise<{ rows: any[]; errors: string[] }> {
  const errors: string[] = [];
  const rows: any[] = [];
  ...
  const tasks = teams.map((team: any) => async () => {
    ...
    const [statsRes, recordRes] = await Promise.all([
      safeFetch(`${ESPN_SITE}/${league.sport}/${league.espnSlug}/teams/${eid}/statistics`, `stats-${eid}`),
      safeFetch(`${ESPN_SITE}/${league.sport}/${league.espnSlug}/teams/${eid}/record`, `rec-${eid}`),
    ]);
...
Deno.serve(async (req: Request) => {
  ...
  const t0 = Date.now();
  const summary: Record<string, any> = { success: true, version: 'v4', dryRun, leagues: leagueKeys, drains: {} };
  ...
  try {
    for (const dtype of drainTypes) {
      ...
      for (const leagueKey of leagueKeys) {
        ...
        if (!league) { dr.errors.push(`Unknown league: ${leagueKey}`); continue; }

        L.info('DRAIN_START', { type: dtype, league: leagueKey, version: 'v4' });

        let result: { rows: any[]; errors: string[] };
        switch (dtype) {
          case 'leaders': result = await drainLeaders(supabase, leagueKey, league); break;
          case 'team_stats': result = await drainTeamStats(supabase, leagueKey, league); break;
      ...
      if (!dryRun) {
        await supabase.from('espn_stats_drain_log').insert({
          ...
          errors: dr.errors.slice(0, 50), duration_ms: Date.now() - t0,
          drain_version: 'v4',
